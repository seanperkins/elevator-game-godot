...
about 0.51 on an iPhone 15 — so a row is roughly **16.5 pt**, and under 14 pt on
an iPhone SE or once the HUD takes its share. That is 2.5–3x under Apple's ~44 pt
...
**The rail's initial detent is the car's current row, not the pressed row.**
This matters: if the initial detent were the press point, then "press on row R and
release" would be indistinguishable from a tap, so dispatching a car to the row
your thumb is already on — the most common intent, "there is a passenger *there*"
— would require dragging away and back. Anchoring at the car's current row makes
release-in-place a genuine no-op, so classifying it as surge is unambiguous, and
dispatching to any row including the pressed one is always a real drag.
...
- **8 shafts maximum.** At a 720-unit base width, 44 pt is about 85 units on an
  iPhone 15, so the screen fits 8 columns edge-to-edge and realistically **6–7**
  ...
  cap; capacity growth past that point comes from **cars per shaft**, speed,
  capacity, and door time, which the express-shaft and zoning mechanics (§6, §5.4)
  support without needing more columns.
...
*unachievable at any integer row count* for Orbital Tether, which would need half
a row. What resets, and what carries the felt reset, is the **board** (6 of 40
...
**The JS callback fires outside the Godot main loop.** It sets a flag consumed at
the top of the next `_physics_process`; it never mutates sim state inline.
...
`sim_wall_time` advances only by *executed* ticks (`ticks_executed x 0.05`).
Everywhere — resume, cold start, after a hitch — the window is:
...
**Discrete state is reconciled at the boundary, in both directions.** On Hidden,
the waiting passenger population is cleared and folded into the catch-up window's
statistics, and cars are parked idle at their last stop. Without this, resume
restarts the live sim from a discrete state whose patience timers are on a
seconds-to-minutes scale but which is now hours old — so the first live ticks after
*every* resume produce a mass-expiry event: satisfaction craters and the combo
dies, punishing the player for the single most common daily action. The
welcome-back summary would report a healthy window and then the screen would fill
with expiring passengers. **Where §8.6's continuation state and the catch-up model
disagree, catch-up wins on the Hidden→Resumed path**; continuation state exists so
a *cold start with no elapsed time* resumes identically, not to survive catch-up.

**Other rules:**

- Offline rate is never the instantaneous rate at save time.
- Base offline rate is **100% of the analytic model**; the Automation branch
  raises the *cap*, not the rate. (An upgradeable rate fraction would make offline
  differ from live by design and muddle §9's fidelity test.)
...
- Catch-up runs strictly after §8.6 step 6, on validated values — `clampf(NAN, …)`
...
- Export is `Marshalls.utf8_to_base64(JSON.stringify(state))`; import is
  `base64_to_utf8()` then `JSON.parse_string()`. **`variant_to_base64` and
  `bytes_to_var` are forbidden on both sides** — the compact-looking option is
  exactly the call §8.6 bans, and banning it only on the load side leaves the trap
  armed on the export side.
- A hard byte cap enforced *before* base64-decode, and a decoded-size cap before
  parse. Godot's JSON parser is recursive-descent, so an over-long or deeply
  nested paste is a memory kill or stack overflow — a hard crash, which means the
  "reject cleanly" path never runs.
...
That is thousands, not hundreds — roughly 2.8x an earlier estimate that assumed one
node per passenger, and past the threshold where Control-node layout cost on GL
Compatibility in mobile Safari stops being free. The per-row cap is therefore a
**tunable constant with a global node ceiling above it**, and ~1,700 nodes is
measured on the target iPhone before Milestone 3. The car term is bounded only
because §3 caps shafts.
...
3. Read and check `version` before interpreting any other field. A version above
   the newest known, *below the oldest migration floor*, absent, or non-integer all
   route through the same refuse-and-backup path.
...
the immediate hidden-save. Only an explicit user action clears the latch —
"discard the unreadable save and start fresh," or a successful validated import
(which must remain available while the latch is set, since import is the recovery
path). The blocking message names the backup file, since a web player cannot browse
`user://`.
...
**Test B — economic fidelity and the no-exploit property.** Catch-up earnings over
any window are within a stated engineering budget (±5%) of the mean of N seeded
live runs, and — the property that actually matters — `max over t₀ of
(catch-up / lived) <= 1.0 + δ`. A ratio bound is what protects the economy and is
what the round-1 exploit finding was really about.
...
14. Hostile-input matrix: wrong type per schema field, `1e400`, negative counts,
    ...
    deep nesting, over-length import string, non-base64 import string, `NAN` in
    `saved_at`.
