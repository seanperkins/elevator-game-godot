...
func before_each() -> void:
	...
	for car in gs.building.cars:
		car.floors_per_tick = 1.0               # a floor a tick, so tests are short
		car.door_ticks = 2
...
## Runs until the named car reaches `floor`, or gives up. Returns the ticks taken.
func run_until_at(shaft: int, floor_index: int, limit := 400) -> int:
	var car: ElevatorCar = gs.building.cars[shaft]
	for t in range(limit):
		gs.tick(1)
		if car.current_floor() == floor_index:
			return t
	return -1
...
func test_a_manual_car_never_moves_on_its_own() -> void:
	licence()
	gs.tick(200)
	assert_almost_eq(gs.building.cars[0].position_floor, 0.0, 1e-9,
...
func test_it_visits_every_floor_in_turn() -> void:
	...
	for t in range(600):
		...
		if car.state == ElevatorCar.State.DOORS:
			seen[car.current_floor()] = true
	for floor_index in range(gs.building.floor_count):
		assert_true(seen.has(floor_index), "floor %d must get a visit" % floor_index)

func test_it_turns_round_at_the_top_instead_of_leaving_the_building() -> void:
	licence()
	assert_true(gs.set_auto(0, true))
	var car: ElevatorCar = gs.building.cars[0]
	var top := gs.building.floor_count - 1
	assert_ne(run_until_at(0, top), -1, "reached the top")
	for t in range(200):
		gs.tick(1)
		assert_between(car.position_floor, 0.0, float(top),
			"stayed inside the building")
		if car.current_floor() < top:
			return
...
func test_it_turns_round_at_the_lobby_too() -> void:
	...
	assert_true(gs.set_auto(0, true))
	var car: ElevatorCar = gs.building.cars[0]
	assert_ne(run_until_at(0, gs.building.floor_count - 1), -1)
...
func test_only_the_enabled_shaft_moves() -> void:
	...
	assert_true(gs.set_auto(0, true))
	gs.tick(200)
	assert_gt(gs.building.cars[0].position_floor, 0.0, "shaft 0 is sweeping")
	assert_almost_eq(gs.building.cars[1].position_floor, 0.0, 1e-9,
...
func test_switching_it_back_off_stops_it_where_it_stands() -> void:
	...
	var resting := gs.building.cars[0].position_floor
	gs.tick(200)
	assert_almost_eq(gs.building.cars[0].position_floor, resting, 1e-9,
...
func test_it_does_not_steal_a_car_in_transit() -> void:
	...
	assert_true(gs.set_auto(0, true))
	gs.tick(1)
	assert_eq(car.target_floor, 5, "the trip the player asked for is not overridden")
...
func test_a_manual_dispatch_still_works_and_the_sweep_resumes() -> void:
	...
	gs.tick(40)
	gs.dispatch(0, 4)
	assert_eq(gs.building.cars[0].target_floor, 4, "the player still has the wheel")
...
func test_a_one_floor_building_does_not_thrash() -> void:
	...
	solo.tick(100)
	assert_almost_eq(solo.building.cars[0].position_floor, 0.0, 1e-9,
...
func test_a_call_policy_only_visits_floors_with_someone_on_them() -> void:
	...
	for t in range(300):
		gs.tick(1)
		if car.state == ElevatorCar.State.DOORS:
			visited[car.current_floor()] = true
...
func test_lobby_parking_brings_an_idle_car_home() -> void:
	...
	gs.building.cars[0].position_floor = 5.0
	gs.building.cars[0].target_floor = 5
...
func test_without_the_spring_a_lobby_to_top_run_is_ordinary() -> void:
	...
	gs.tick(10)
	assert_almost_eq(car.position_floor, 1.0, 1e-6, "one floor_index in ten ticks")
...
func test_the_spring_launches_the_lobby_to_top_run() -> void:
	...
	assert_true(gs.dispatch(0, 5))
	gs.tick(10)
	assert_almost_eq(car.position_floor, 4.0, 1e-6, "four times the speed")
...
func test_the_spring_only_serves_the_lobby_to_top_run() -> void:
	...
	gs.tick(10)
	assert_almost_eq(car.position_floor, 1.0, 1e-6)
...
func test_a_launched_car_cannot_be_stopped_on_the_way() -> void:
	...
	assert_eq(car.target_floor, 5, "still bound for the top")
...
func test_a_launched_car_is_free_again_once_it_arrives() -> void:
	...
	for t in range(60):
		gs.tick(1)
		if car.current_floor() == 5:
			break
	assert_eq(car.current_floor(), 5, "arrived")
