...
func test_the_content_follows_the_thumb() -> void:
	...
	g.press(300.0, 0)
	g.move(260.0)
	assert_almost_eq(g.take_pan_delta(), 40.0, 1e-6, "thumb up 40, content up 40")
	g.press(300.0, 0)
	g.move(340.0)
	assert_almost_eq(g.take_pan_delta(), -40.0, 1e-6, "and down the other way")
...
func test_a_pan_reports_only_what_changed_since_it_was_last_asked() -> void:
	...
	g.press(300.0, 0)
	g.move(260.0)
	assert_almost_eq(g.take_pan_delta(), 40.0, 1e-6)
	g.move(250.0)
	assert_almost_eq(g.take_pan_delta(), 10.0, 1e-6, "10 more, not 50")

func test_taking_the_delta_consumes_it() -> void:
	g.press(300.0, 0)
	g.move(260.0)
	g.take_pan_delta()
	assert_almost_eq(g.take_pan_delta(), 0.0, 1e-9,
...
func test_movement_before_the_threshold_is_not_thrown_away() -> void:
	...
	g.press(300.0, 0)
	g.move(300.0 - Gesture.DRAG_THRESHOLD - 1.0)
	assert_almost_eq(g.take_pan_delta(), Gesture.DRAG_THRESHOLD + 1.0, 1e-6)
...
func test_a_second_press_starts_clean() -> void:
	...
	assert_almost_eq(g.take_pan_delta(), 0.0, 1e-9, "no carry-over from the last drag")
	assert_eq(g.release(), Gesture.Result.TAP)
